...
const App: React.FC = () => {
  ...
  const handleNewDrawing = useCallback(() => {
    if (window.confirm('Clear board and start a new session?')) {
      generateNewSession();
    }
  }, [generateNewSession]);
    ...
    <div className="fixed inset-0 bg-[#000000] flex flex-col overflow-hidden select-none touch-none">
      <div className="absolute top-4 left-4 z-10 pointer-events-none flex flex-col gap-1.5">
        <h1 className="text-white font-bold text-2xl tracking-tighter">
          DRAWBN<span className="text-purple-500 font-normal">.ONLINE</span>
        </h1>
        <div className="flex items-center gap-2">
          <div className="bg-zinc-900/90 backdrop-blur-xl px-3 py-1.5 rounded-lg border border-zinc-800 flex items-center gap-2.5 shadow-2xl">
            <span className="text-zinc-500 text-[10px] font-black tracking-widest uppercase">ID: {sessionId}</span>
            <div className="w-[1px] h-3 bg-zinc-700" />
            <div className="flex items-center gap-1.5">
              <div className={`w-2 h-2 rounded-full transition-all duration-500 ${
                saveStatus === 'saving' ? 'bg-amber-400 animate-pulse' : 
                saveStatus === 'saved' ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'bg-zinc-600'
              }`} />
      ...
      {showShareModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-[2.5rem] w-full max-w-md shadow-2xl text-center">
            ...
            <p className="text-zinc-400 text-sm mb-8 px-4">Copy the link below to share this blackboard. Others will see your drawing exactly as it is.</p>
            <div className="relative mb-8">
              ...
              <button onClick={() => { navigator.clipboard.writeText(shareUrl); alert('Link copied!'); }} className="absolute right-2 top-2 bottom-2 px-6 bg-white text-black rounded-xl text-[10px] font-black uppercase transition-transform active:scale-95 shadow-lg">Copy</button>
            </div>
